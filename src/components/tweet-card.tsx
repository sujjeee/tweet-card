...
import { Tweet, TweetPhoto } from "react-tweet/api"
import { enrichTweet } from "react-tweet"
import { cn } from "@/lib/utils"

interface TweetCardProps extends React.HTMLAttributes<HTMLDivElement> {
  tweetPromise: Promise<Tweet | undefined>
  metrics?: boolean
  media?: boolean
  timestamp?: boolean
  autoPlay?: boolean
...
export function TweetCard(props: TweetCardProps) {
  ...
  const tweet = React.use(tweetPromise)
        ...
        {media && tweet.mediaDetails && (
          <div className="relative overflow-hidden mt-3 rounded-xl  ">
            ...
            {tweet.photos && tweet.photos.length > 0 && (
              <TweetPhotos tweet={tweet} tweetPhotos={tweet.photos} />
            )}
          </div>
        )}
...
function TweetVideo({ poster, source, autoPlay }: TweetVideoProps) {
...
interface TweetPhotosProps {
  tweet: Tweet
  tweetPhotos: TweetPhoto[]
}

function TweetPhotos({ tweet, tweetPhotos }: TweetPhotosProps) {
  return (
    <>
      ...
      {tweetPhotos.length > 1 ? (
        <div className="grid grid-cols-[1fr_1fr] overflow-hidden aspect-[1.77_/_1] gap-[1.6px]">
          {tweetPhotos.map((photo, idx) => (
            <div
              className={cn(
                "overflow-hidden",
                tweetPhotos &&
                  tweetPhotos.length > 2 &&
                  idx === 0 &&
                  "row-span-2",
              )}
              key={photo.url}
